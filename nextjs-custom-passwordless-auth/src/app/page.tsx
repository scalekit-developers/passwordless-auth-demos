...
export default function LoginPage() {
  ...
  async function handleOtpSubmit(e: React.FormEvent) {
    e.preventDefault();
        ...
        try {
          const result = await signIn("scalekit", {
            redirect: false,
            email,
            code: otp,
            authRequestId,
          ...
          if (result?.ok && !result.error) {
            ...
            const msg = result?.error || "Invalid or expired code. Please check and try again.";
            if (msg.toLowerCase().includes("expired")) {
              setError("Your login session has expired. Please request a new code or magic link.");
            } else if (msg.toLowerCase().includes("verification failed")) {
